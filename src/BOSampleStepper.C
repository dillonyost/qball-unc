...
void BOSampleStepper::step(int niter)
{
  ...
  HPM_Stop("scfloop");
        ...
        if ( compute_mlwf || compute_mlwfc )
        {
           ...
           if (wf.force_complex_set() ) tdmlwft->compute_transform();
	   else mlwft->compute_transform();

          if ( compute_mlwf )
	   {
	    if (wf.force_complex_set() ) tdmlwft->apply_transform(sd);
            else mlwft->apply_transform(sd);
           }
          if ( onpe0 )
          {
	    cout << "DCY " << wf.force_complex_set() << endl;
            cout << " <mlwf_set size=\"" << sd.nst() << "\">" << endl;
            for ( int i = 0; i < sd.nst(); i++ )
            {
	      if (wf.force_complex_set() )
	      {
		if (onpe0) cout << "DCY check" << endl; 
