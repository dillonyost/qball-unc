                 ...
                 for ( int l = 0; l < (ortho.mblocks()); l++ ) {
                  for ( int y = 0; y < ortho.nbs(m); y++ ) {
                   for ( int x = 0; x < ortho.mbs(l); x++ ) {
                    ...
                    oss << itmp << " " << jtmp << " " << ival << " " << fixed <<  ortho[ival] << "  " << fixed << norm(ortho[ival]) << endl;
...
// if saveprojfreq variable set, save density in VMD Cube format
/*
if (s_.ctrl.saveprojfreq > 0)
    {
     for ( int ispin = 0; ispin < (wf).nspin(); ispin++ )
...
//          std::ofstream os;
          std::ostringstream oss;
          oss.width(8);  oss.fill('0');  oss << s_.ctrl.mditer;
          std::string projfilename = filebase + "." + oss.str() + ".txt";
          std::string format = "text";
//          os.open(projfilename.c_str(),ofstream::out);    // text output
//          os.setf(ios::fixed,ios::floatfield);
//          os << setprecision(8);
