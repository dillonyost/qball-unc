    ...
    for ( int i = 0; i < a[k]->nloc(); i++ )
     {
      ...
      //if (ctxt.onpe0()) cout << k << " " << i << " " << (*acol[k][i]) << endl;
     }
...
*/
      ...
      for ( int k = 0; k < a.size(); k++ )
      {
        for ( int ipair = 0; ipair < nploc; ipair++ )
        {
          ...
          if ( jglobal[top[ipair]] >= 0 && jglobal[bot[ipair]] >= 0 )
          {
            ...
            //cout << "apq = " << ctxt.mype() << " " << apq[iapq] << endl;
          }
        }
      } // for k
      // apq now contains partial sums of matrix elements
      vector <double> tapq(apq);
      tm_comm.start();
      int len = apq.size();
      ctxt.dsum('c',len,1,&apq[0],len);
      tm_comm.stop();
      //cout << "apq = " << ctxt.mype() << " "  << (apq[0]) << endl;
	  ...
	  */
          ...
          for ( int k = 0; k < a.size(); k++ )
          {
            ...
            //if (ctxt.onpe0()) cout << "acol " << k << " " << top[ipair] << " " << (*acol[k][top[ipair]]) << endl;
          }
          //if (ctxt.onpe0()) cout << "ucol top " << (*ucol[1]) << endl;
          double *up = ucol[top[ipair]];
          //if (ctxt.onpe0()) cout << "ucol bot " << (*ucol[63]) << endl;
          double *uq = ucol[bot[ipair]];
          int mloc = u.mloc();
          //if (ctxt.onpe0()) cout << "ucol 1 " << (*ucol[1])  << endl;
          drot(&mloc,up,&one,uq,&one,&c,&s);
          //if (ctxt.onpe0()) cout << "ucol 1 " << (*ucol[0])  << endl;
          ...
          for ( int k = 0; k < a.size(); k++ )
          {
            ...
            const double app = apq[iapq+1];
            const double aqq = apq[iapq+2];
	    const double apqnew = (c*c-s*s)*(app - aqq) + 4*c*s*apq[iapq];
	    //if (ctxt.onpe0()) cout << "apqnew " << k << " " << apqnew << endl;
            diag_change += abs(apqnew - (app - aqq));
            //diag_change -= 2.0 * ( apqnew*apqnew - apq[iapq]*apq[iapq] );
          }
        }
      } // for ipair
        //if (ctxt.onpe0()) cout << "diag_change = " << diag_change << endl;
	  ...
	  //cout << "sbufi_right = " << sbufi_right << endl;
        ...
        if ( ctxt.mycol() < last_active_process_col && bufsize > 0)
        {
          ...
          memcpy(ucol[bot[nploc-1]], &rbuf_right[a.size()*mloc],
                 mloc*sizeof(double) );
          //cout << ctxt.mype() << ": received u= " << (*ucol[bot[nploc-1]]) << " from right" << endl;
        ...
        if ( ctxt.mycol() != 0 && bufsize > 0 )
        {
          ...
          //cout << ctxt.mype() << ": received u= " << (*ucol[bot[nploc-1]]) << " from left" << endl;
        }
        tm_comm.stop();
      } // if nploc > 0
